...
const props = defineProps({
  ...
  label: { type: String, default: "Vai trò:" },
  name: { type: String, default: "role_id" },
  multiple: { type: Boolean, default: false },
  placeholder: { type: String, default: "Chọn vai trò" },
  size: { type: String, default: "middle" },
  rules: { type: Array, default: () => [] },
  disabled: { type: Boolean, default: false },
  /** Tự động chọn vai trò đầu tiên nếu chưa chọn giá trị */
  ...
  /** Hiển thị nút chuyển sang vai trò trước/sau */
...
// Sử dụng pattern reactive data fetching của Nuxt
const {
  data: roleResponse,
  ...
  params: params,
  key: "admin-roles-select",
});

const options = computed(() => {
  if (!roleResponse.value?.success) return [];
  const items = Array.isArray(roleResponse.value.data?.items) ? roleResponse.value.data.items : [];
