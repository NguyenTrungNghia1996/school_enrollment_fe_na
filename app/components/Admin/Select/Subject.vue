...
const props = defineProps({
  ...
  name: { type: String, default: "subjectId" },
  multiple: { type: Boolean, default: false },
  placeholder: { type: String, default: "Chọn môn học" },
  size: { type: String, default: "middle" },
  rules: { type: Array, default: () => [] },
  disabled: { type: Boolean, default: false },
  /** Tự động chọn môn học đầu tiên nếu chưa chọn giá trị */
...
const {
  ...
  params: params,
  key: "admin-subjects-select",
});

const options = computed(() => {
  if (!subjectResponse.value?.success) return [];
  const items = Array.isArray(subjectResponse.value.data?.items) ? subjectResponse.value.data.items : [];
