  ...
  <div class="w-full bg-slate-50 pb-20">
    ...
    <section class="relative overflow-hidden bg-gradient-to-br from-[#071f41] via-[#0b2b5c] to-[#051630] py-20 text-white shadow-lg">
      ...
      <div class="container relative z-10 mx-auto px-4 text-center sm:px-6 lg:px-8">
        <span class="mb-4 inline-flex items-center gap-1.5 rounded-full border border-primary/30 bg-primary/20 px-3 py-1 text-xs font-bold uppercase tracking-wider text-blue-300 backdrop-blur-sm">
          <Icon name="lucide:info" />
          HỖ TRỢ THÍ SINH
        </span>
        <h1 class="flex items-center justify-center gap-3 text-3xl font-black tracking-tight sm:text-4xl lg:text-5xl">
          <Icon name="lucide:book-open" class="text-4xl text-primary lg:text-5xl" />
          Hướng Dẫn Sử Dụng
        </h1>
        <p class="mx-auto mt-4 max-w-2xl text-base leading-relaxed text-blue-100/80 lg:text-lg">
    ...
    <div class="container mx-auto mt-12 px-4 sm:px-6 lg:px-8">
      <div class="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_320px]">
        ...
        <div class="space-y-12">
          <article v-for="section in sections" :key="section.id" :id="section.id" class="section-element scroll-mt-32 rounded-2xl border border-slate-100 bg-white p-6 shadow-[0_4px_20px_rgba(0,0,0,0.02)] transition-all duration-300 hover:border-slate-200/80 hover:shadow-[0_12px_40px_rgba(0,0,0,0.06)] md:p-8">
            <!-- Section Header -->
            ...
            <div class="space-y-6">
              <template v-for="(block, index) in section.blocks" :key="index">
                <!-- Paragraph block -->
                <p v-if="block.type === 'p'" class="text-justify text-base leading-relaxed text-slate-600">
                  {{ block.content }}
                </p>

                <!-- Step block -->
                <div v-else-if="block.type === 'step'" class="flex gap-4 rounded-2xl border border-slate-100 bg-slate-50/70 p-5 transition-all duration-200 hover:bg-slate-50">
                  <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gradient-to-tr from-primary to-blue-600 text-sm font-bold text-white shadow-md shadow-primary/20">
                    {{ block.number }}
                  </div>
                  <div class="pt-0.5">
                    <span class="text-base font-extrabold text-slate-800">Bước {{ block.number }}:</span>
                ...
                <div v-else-if="block.type === 'note'" class="flex gap-4 rounded-r-2xl border-l-4 border-amber-500 bg-amber-50/40 p-5 text-amber-900 shadow-sm">
                  <Icon name="lucide:alert-circle" class="mt-0.5 shrink-0 text-2xl text-amber-600" />
                  <div class="text-sm leading-relaxed">
                    <strong class="mb-1 block font-bold text-amber-800">Lưu ý quan trọng:</strong>
                    {{ block.content }}
                  </div>
                </div>

                <!-- Image block -->
                <div v-else-if="block.type === 'image'" class="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200/80 bg-slate-50/30 px-4 py-6">
                  <div class="max-w-full overflow-hidden rounded-xl border border-slate-200 bg-white shadow-md transition-all duration-300 hover:shadow-xl">
                    <a-image :src="block.src" :alt="block.caption" class="max-h-[460px] w-auto max-w-full cursor-zoom-in object-contain" />
                  </div>
                  <span v-if="block.caption" class="mt-3 flex items-center gap-1 text-xs font-bold uppercase tracking-wider text-slate-400">
          ...
          <div class="sticky top-[120px] space-y-6">
            <div class="rounded-2xl border border-slate-100 bg-white/90 p-6 shadow-[0_8px_30px_rgb(0,0,0,0.02)] backdrop-blur-md">
              ...
              <nav class="space-y-1.5">
                <button v-for="item in sections" :key="item.id" @click="scrollToSection(item.id)" class="flex w-full items-center gap-3 rounded-xl px-4 py-3.5 text-left text-sm font-semibold transition-all duration-300" :class="activeSectionId === item.id ? 'translate-x-1 bg-primary font-bold text-white shadow-lg shadow-primary/20' : 'text-slate-500 hover:translate-x-0.5 hover:bg-slate-50 hover:text-slate-900'">
            ...
            <div class="group relative overflow-hidden rounded-2xl bg-slate-900 p-6 text-white shadow-xl">
              <div class="absolute -bottom-10 -right-10 h-32 w-32 rounded-full bg-primary/10 blur-xl transition-all duration-500 group-hover:bg-primary/20"></div>
              <h3 class="mb-2 flex items-center gap-2 text-base font-bold">
                <Icon name="lucide:headphones" class="text-lg text-primary" />
                Cần hỗ trợ?
              </h3>
              <p class="mb-4 text-xs leading-relaxed text-slate-400">Nếu bạn gặp khó khăn trong quá trình đăng ký, hãy liên hệ với chúng tôi qua các kênh bên dưới.</p>
              <div class="relative z-10 space-y-3.5">
                <div class="flex items-center gap-3 transition-transform duration-200 hover:translate-x-0.5">
                ...
                <div class="flex items-center gap-3 transition-transform duration-200 hover:translate-x-0.5">
                  ...
                  <div class="min-w-0">
                    ...
                    <p class="truncate text-xs font-bold text-slate-100">{{ unitStore.email || "Chưa cập nhật" }}</p>
                  </div>
                </div>
              </div>
            </div>
    ...
    <div class="fixed bottom-6 right-6 z-50 lg:hidden">
      <a-popover trigger="click" placement="topRight" overlayClassName="mobile-toc-popover">
        <template #content>
          <div class="max-h-[360px] w-64 overflow-y-auto p-2">
            <h4 class="mb-3 flex items-center gap-2 border-b border-slate-100 px-2 pb-2 text-sm font-bold text-slate-800">
              <Icon name="lucide:list" />
              Chỉ mục nhanh
            </h4>
            <nav class="space-y-1">
              <button v-for="item in sections" :key="item.id" @click="scrollToSection(item.id)" class="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left text-xs font-semibold transition-all" :class="activeSectionId === item.id ? 'bg-primary/10 text-primary' : 'text-slate-500 hover:bg-slate-50'">
        ...
        <button class="flex h-14 w-14 shrink-0 cursor-pointer items-center justify-center rounded-full border-none bg-gradient-to-r from-primary to-blue-600 p-0 text-white shadow-2xl transition-all duration-200 hover:scale-105 hover:shadow-primary/30 focus:outline-none active:scale-95">
...
const allSections = [
  ...
  },
  {
    ...
    blocks: [
      ...
      { type: "note", content: "Mỗi tài khoản chỉ được phép đăng ký một hồ sơ dự đánh giá năng lực trong một kỳ tuyển sinh. Nếu muốn đăng ký nhiều hồ sơ, thí sinh cần sử dụng các tài khoản khác nhau." },
    ],
  },
  {
    id: "payment",
    ...
    paymentRelated: true,
    blocks: [
      { type: "p", content: "Sau khi hồ sơ được xác nhận hợp lệ, thí sinh thực hiện thanh toán lệ phí dự đánh giá năng lực theo các bước sau:" },
